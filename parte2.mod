...
s.t. localizacion_seleccionada {l in Localizaciones}: 
	sum{d in Distritos} x[l, d] >= 0 - M * (1 - Loc_construidas[l]);
...
s.t. binario_75_part1 {d in Distritos}: 
	sum {l in Localizaciones} x[l, d]  >= - M * (1 - b2[d]) + (Max_distribucion * Max_llamadas);
s.t. binaria_75_part2 {d in Distritos}: 
	sum {l in Localizaciones} x[l, d] <= b2[d] * M + (Max_distribucion * Max_llamadas) - 1;
